...
7. Reconcile the source-aware argument map with the source-blind reconstruction.
   ...
   most twice. Rerun both reviews after each change. After the second revision,
   unresolved blocking findings stop the unit for user judgment; non-blocking
   style suggestions are recorded and the unit may advance.
...
10. Before registering any candidate version, send only that assembled Chinese
    ...
    recheck, second repair and final recheck. A third blocking `FAIL` stops
    automatic revision and hands the unresolved passages to the user; it does
    not trigger a third repair. A `PASS` under
    `references/review-protocol.md` is still required for registration.
