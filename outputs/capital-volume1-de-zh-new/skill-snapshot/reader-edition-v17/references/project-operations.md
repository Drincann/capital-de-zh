...
the third review still finds a blocking translation defect, stop automatic
revision and hand the exact finding to the user. Style suggestions never consume
a return and never block registration when the current wording is already clear.
...
7. report exact artifacts and the next pending unit.
