...
three attempts with `-r1`, `-r2`, and `-r3` in their filenames. If the final
recheck still finds a `T`, stop automatic rewriting and hand the exact passage
and finding to the user. Do not start a third revision.
...
stops automatic work and is handed to the user; it never triggers a third
repair. A passing review is still required before registration.
