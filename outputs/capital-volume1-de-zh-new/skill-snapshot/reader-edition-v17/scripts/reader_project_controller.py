...
STANDARD_VERSION = "reader-edition-v17"
SPEC_FILES = (
...
7. Before registering a candidate version, require a hash-bound PASS from an
   ...
   not fail it. A third blocking FAIL stops automatic work for user judgment.
...
- every registered candidate has an independent-reader PASS bound to its exact
  assembled text, using material translation clarity rather than theoretical
  agreement or stylistic perfection as the criterion;
...
10. A task may be returned for revision at most twice. After the second revision
    and final recheck, stop on any remaining blocking defect and ask the user.
