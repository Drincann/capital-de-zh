...
3. Draft the shortest natural Chinese that lets a new reader follow the argument
   as one continuous thought. Preserve source paragraph boundaries by default.
   Reorder sentences, name implicit subjects, and unpack compressed reasoning as
   needed; split or join paragraphs only when necessary for comprehension.
...
5. Run a meaning audit: core claims, logic, concepts, facts, figures, and
   quotations.
6. Run a source-blind readability and continuity audit. Rewrite anything a
   normal reader must decode twice, any silent change of conceptual subject, and
   any paragraph that reads as a list rather than a line of reasoning. Audit every
   changed paragraph boundary and reject changes made only for visual rhythm or
   shorter blocks. Replace any semicolon that merely compresses two sentences or
   imitates the source's long syntax.
