...
- Classify every finding as `T` (translation obstacle), `A` (authorial
  difficulty), or `D` (question deferred by the source). Only unresolved `T`
  findings block a pass. A source-blind reviewer who cannot classify a finding
  must flag it for the later source-aware comparison rather than demand an
  explanatory rewrite.
...
- a noticeably shorter or more concrete formulation preserves the same source
  content, but the draft keeps a longer abstract formulation merely because it
  resembles the source wording;
...
place where the Chinese made it necessary to reread, infer an unstated textual
bridge, guess a referent or comparison object, or rely on specialist vocabulary.
The reviewer does not judge whether the source has proved its claim. Questions
about truth, proof, or later theory may be reported as `A` or `D`, but they do
not force failure. Only unresolved `T` obstacles force `Verdict: FAIL`.
