...
STANDARD_VERSION = "reader-edition-v16"
SPEC_FILES = (
...
7. Before registering a candidate version, require a hash-bound PASS from an
...
- every registered candidate has an independent-reader PASS bound to its exact
  assembled text, using translation clarity rather than theoretical agreement as
  the criterion;
...
def review_errors(
    ...
    if re.search(r"\bTODO\b|\bTBD\b|待补|占位", text, re.IGNORECASE):
    ...
    if "- None in current draft." not in review_section(
        text, "Second-read risks"
    ):
        errors.append(
            "a passing readability review must state "
            "'- None in current draft.' under Second-read risks"
        )
    semicolon_match = re.search(
        r"^Semicolons:\s*(\d+)\s*$", punctuation, re.MULTILINE
    )
    if not semicolon_match:
        errors.append("Paragraph and punctuation audit must record Semicolons")
    elif int(semicolon_match.group(1)) != draft_text.count("；"):
        errors.append("recorded semicolon count does not match the draft")
    if not re.search(
        r"^Boundary-Changes:\s*\S", punctuation, re.MULTILINE
    ):
        errors.append(
...
def independent_review_errors(
    ...
    if review_field(text, "Verdict") != "PASS":
        errors.append("independent reader Verdict must be PASS")
    ...
    project.update(
        {
            ...
            "translation_policy": (
                "忠实于德文的命题、逻辑、概念区别、例子和顺序，而不追求词语相似；"
                "默认使用最短、最直接的自然中文，只有直接表达仍无法理解时才深度重构；"
                "不得替原文补证明、提前展开后文或改变核心命题和概念区别"
            ),
...
翻译忠实于德文的命题、关系、概念区别、顺序、例子、限定和历史细节，
不追求词语或句式相似。默认写成最短、最直接的自然中文；词类转换、把
抽象名词改成动作、删除无意义的重复都属于正常翻译。只有直接表达仍无
法让普通读者理解时，才作意义导向的深度重构。不得替原文补证明、提前
展开后文，或改变核心命题、关系、概念、事实、数字和引文。
...
def refresh_tasks(root: Path, chapter_id: str) -> None:
    chapter_rows = read_jsonl(root / "manifests" / "chapters.jsonl")
    chapter = next(
        (row for row in chapter_rows if row.get("chapter_id") == chapter_id), None
    )
...
def register_version(
    ...
    failures = independent_review_errors(
        unit_id, version_text, reader_review
    )
    if failures:
        raise SystemExit(
...
def main() -> int:
    ...
    if args.command == "migrate":
        ...
        refresh_tasks(root, args.chapter)
    elif args.command == "revise-tasks":
        revise_tasks(root, args.chapter, args.allow_incomplete)
