...
For a book-wide recall caused by a readability-rule change, first extract the
...
capture; a source-blind reader still decides whether each candidate is a real
`T`, `S`, `A`, or `D` finding. Never treat lexical matching as a verdict.
