...
This is a source-near modern translation, not a line-by-line crib and not a
commentary. Preserve the source's claims, sequence, examples, qualifications,
paragraphs, and rhetorical movement by default. First repair only what ordinary
Chinese requires: syntax, explicit subjects, punctuation, reference, and visible
logical connections. Use freer meaning-centered reconstruction only when those
local repairs still leave a normal reader unable to understand the source.
...
- Stay as close to the source as natural Chinese allows. Do not expand a passage
  merely because its theory is difficult, disputable, compressed, or completed
  later in the book.
...
3. Draft by the minimum-intervention ladder. First retain the source sentence
   order and repair Chinese syntax. If that fails, split or locally reorder
   sentences inside the source paragraph. Only if the passage remains
   unintelligible may you reconstruct its expression more freely. Preserve
   source paragraphs, examples, qualifications, and detail unless a specific
   obstacle justifies changing them.
...
6. Run a separate source-blind readability and continuity audit. Do not merely
   ...
   boundary and reject changes made
   only for visual rhythm or shorter blocks. Replace any semicolon that merely
   compresses two sentences or imitates the source's long syntax.
