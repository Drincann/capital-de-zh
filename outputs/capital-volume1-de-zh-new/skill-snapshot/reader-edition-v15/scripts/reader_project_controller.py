...
STANDARD_VERSION = "reader-edition-v15"
SPEC_FILES = (
    ...
    project.update(
        {
            ...
            "translation_policy": (
                "贴近德文原文是默认；先用自然中文修复句法、指代和衔接，"
                "只有这些局部处理仍无法理解时才作意义导向的深度重构；"
...
翻译默认贴近德文原文，保留命题顺序、段落、例子、限定和历史细节。先
只调整德语句法、隐含主语、指代、标点和中文衔接；只有这些局部处理仍
无法让普通读者理解时，才作意义导向的深度重构。不得替原文补证明、
提前展开后文，或改变核心命题、关系、概念、事实、数字和引文。
...
def revise_tasks(root: Path, chapter_id: str) -> None:
...
def main() -> int:
    ...
    if args.command == "migrate":
        ...
        revise_tasks(root, args.chapter)
