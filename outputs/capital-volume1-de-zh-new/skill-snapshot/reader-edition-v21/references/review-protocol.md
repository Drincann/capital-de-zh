...
Under `Authorial stance and rhetorical-function audit`, include one bullet for
every source item:

```text
- [v1-ch01-s01-p0002]: stance=critical; speaker=author; target=classical political economy; force=sharp; function=exposes a method's social limitation; preservation=kept as direct criticism in contemporary Chinese
```

Use `stance=neutral` when the item contains no consequential evaluation, but do
not infer neutrality merely because its factual proposition can be summarized
without the rhetoric. For non-neutral items, record the speaker, target,
direction or kind of evaluation, approximate force, argumentative or diagnostic
function, and how the draft preserves it. A source criticism rendered as generic
confusion, a named social standpoint rendered as an individual mood, irony
rendered as a plain assertion, or praise rendered as neutral description is a
blocking meaning defect when it changes how the passage is meant to be read.
Modern wording may be milder or sharper at the lexical level only when the
overall target, force, and function remain equivalent.

Do not pass a draft with an omission, logical change, unsupported addition, or
unresolved uncertainty. Do not describe a passage as "compressed rhetoric"
without stating what was removed and why it performs no argumentative,
diagnostic, evaluative, or structural work.
...
- For every explanatory sentence added to unpack a term or compressed step,
  record what structure is active immediately before it and where that
  structure closes. If the explanation sits between two halves of a parallel,
  causal, contrastive, inferential, or example sequence, test whether a new
  reader must remember and reconstruct the suspended first half. Treat that as
  a blocking defect when the explanation can instead be integrated into its
  branch or moved outside the completed structure.
...
- an explanatory insertion interrupts an unfinished parallel, causal,
  contrastive, inferential, or example structure and makes the reader recover
  the suspended construction after a detour;
