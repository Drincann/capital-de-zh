...
conceptual distinctions, facts, unjustified additions, speaking position, and
consequential rhetorical force. It is bound to the exact draft SHA-256 and
contains both an argument-map entry and a stance-and-rhetorical-function entry
for every source item. The latter records the speaker, target, force, function,
and preservation of non-neutral passages; `stance=neutral` is recorded
explicitly when no consequential evaluation is present.
...
audit. Findings are classified as blocking translation defects, non-blocking
style suggestions, authorial difficulties, or questions deferred by the source.
Only blocking translation defects fail the draft; the review does not require
...
- explicit user approval for the governing edition style or an active policy
  permitting automatic adoption of passing versions.
...
Every newly registered version must have a hash-bound independent-reader review.
