...
indispensable conceptual distinctions, argument-bearing evidence, and any
authorial stance or rhetorical move that changes how those claims are meant to
be understood. Treat its wording, syntax, sentence order, paragraphing,
repetition, and genuinely nonessential rhetoric as editable material. Write the shortest direct Chinese that lets an
...
3. Draft the shortest direct Chinese that preserves the argument. Freely change
   ...
   qualifications that affect the claim, distinctions used later, and the
   source's consequential stance toward its subject or opponents. A criticism,
   irony, ridicule, skeptical distance, approval, indignation, or deliberate
   change of register is not expendable merely because the bare factual claim
   survives without it. Compress repetition, minor detail, and rhetorical
   ornament only when they carry no separate argumentative, diagnostic, or
   evaluative work. Use source paragraphs as alignment anchors, not as a
   mandatory reader-facing layout.
4. Check relevant authorial witnesses. Keep this comparison backstage.
5. Run a meaning audit: core claims, logic, concepts, facts, figures,
   quotations, speaking position, and consequential rhetorical force. Record a
   source-item argument map and a source-item stance-and-rhetorical-function map,
   then bind the review to the exact draft hash. The reviewer must identify who
   evaluates whom, the direction and strength of the evaluation, and what the
   move does in the argument. Do not accept a neutral paraphrase merely because
   it preserves the surrounding topic.
...
For a book-wide recall caused by a translation or review-rule change, first extract the
current adopted-version candidates with `scripts/audit_adopted_prose.py`. Save
the JSONL under the project's `audits/` directory and bind every semantic audit
report to those adopted version IDs. The script guarantees scope and context
capture; it must emit one scope record for every adopted unit even when no
lexical trigger is present. A source-aware reviewer decides whether content,
logic, speaking position, stance, or rhetorical function was lost; a
source-blind reader separately decides whether the Chinese communicates its own
argument. Never treat lexical matching, sentiment words, or tone intensity as a
verdict.
