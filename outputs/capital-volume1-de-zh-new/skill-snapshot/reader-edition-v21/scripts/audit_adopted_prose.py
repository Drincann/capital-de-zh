#!/usr/bin/env python3
"""Extract complete adopted-version scope plus automated prose candidates."""
...
def main() -> None:
    ...
    for unit_id, version_id in sorted(adoptions.items()):
        ...
        rows.append(
            {
                "kind": "adopted_unit_scope",
                "unit_id": unit_id,
                "version_id": version_id,
                "artifact_path": version["artifact_path"],
                "artifact_sha256": version.get("artifact_sha256", ""),
                "source_task_revisions": version.get("source_task_revisions", []),
                "standard_version": version.get("standard_version", ""),
            }
        )
        ...
        json.dumps(
            {
                ...
                "scope_records": sum(
                    row["kind"] == "adopted_unit_scope" for row in rows
                ),
