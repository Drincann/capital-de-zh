...
STANDARD_VERSION = "reader-edition-v21"
SPEC_FILES = (
...
CONNECTOR_TRIGGER = re.compile(
    r"但是|但|不过|可是|然而|固然|当然|尽管|虽然|因此|所以|因而|于是|"
    r"而且|并且|同时|仍然|反过来|换句话说|也就是说|却"
)
...
def review_errors(
    ...
    current_standard_review = row.get("spec_sha256") == spec_hash(root)

    if review_type == "meaning":
        required_headings = (
            MEANING_REVIEW_HEADINGS
            if current_standard_review
            else tuple(
                heading
                for heading in MEANING_REVIEW_HEADINGS
                if heading != "Authorial stance and rhetorical-function audit"
            )
        )
        ...
        stance_audit = (
            review_section(text, "Authorial stance and rhetorical-function audit")
            if current_standard_review
            else ""
        )
        for source_id in ids:
            ...
            stance_match = re.search(
                ...
                stance_audit,
                re.MULTILINE,
            ) if current_standard_review else None
            if current_standard_review and not stance_match:
                errors.append(f"missing source stance-audit entry: {source_id}")
            elif current_standard_review and stance_match:
                entry = stance_match.group(1)
                if not re.search(r"\bstance=\S", entry):
                    errors.append(
                        f"source stance-audit entry missing stance=: {source_id}"
                    )
                if "stance=neutral" not in entry:
                    for field in (
                        "speaker=",
                        "target=",
                        "force=",
                        ...
                        if field not in entry:
                            errors.append(
                                "source stance-audit entry missing "
                                f"{field}: {source_id}"
                            )
        return errors
    ...
    if current_standard_review:
        connector_summary = re.search(
            r"^-\s*Connector-Pairs:\s*(\S.*)$",
            transitions,
            re.MULTILINE,
        )
        if not connector_summary:
            errors.append(
                "current-standard readability review must record Connector-Pairs"
            )
        else:
            summary = connector_summary.group(1)
            connector_labels = [
                f"C{index}"
                for index, block in enumerate(blocks, 1)
                if CONNECTOR_TRIGGER.search(block)
            ]
            if connector_labels:
                for label in connector_labels:
                    if not re.search(rf"\b{label}\s*=", summary):
                        errors.append(
                            f"Connector-Pairs missing connector paragraph: {label}"
                        )
            elif summary.strip().lower() != "none":
                errors.append(
                    "Connector-Pairs must be 'none' when the draft has no explicit connector"
                )
    ...
    if current_standard_review and not re.search(
        r"^-\s*Relational-First-Use:\s*\S",
        risks,
        re.MULTILINE,
    ):
        errors.append(
            "current-standard readability review must record Relational-First-Use"
        )
    ...
    project.update(
        {
            ...
            "translation_policy": (
                "忠实于德文的核心命题、逻辑关系、必要概念区别、论证性证据，"
                "以及影响理解的作者立场和修辞功能，"
                "不追求词语、句法或局部顺序相似；默认使用最短、最直接的"
                "自然中文，并可重组句子、显化原文蕴含的中间步骤和压缩确实不承担"
                "论证、诊断或评价作用的"
