    ...
    *,
    ...
    def visit(node: ET.Element) -> None:
        ...
        if name == "mfrac":
            ...
            if len(parts) >= 2:
                chunks.append(f"{parts[0]}/{parts[1]}")
            else:
                chunks.append(normalize_space("".join(node.itertext())))
            if node.tail:
                chunks.append(node.tail)
            return
...
def main() -> int:
    ...
    for node in container.iter():
        ...
        if name == "pb" and node.get("ed") != "manuscript":
            ...
            if not text:
                continue
            # MEGAdigital splits a single printed paragraph at page boundaries
            # into cS (start), cM (middle), and cE (end) fragments.  Only cS
            # starts a new source item; both continuation forms belong to the
            # preceding paragraph.
            if (
                (
                    node.get("n") in {"cM", "cE"}
